# cifar-10 training with sphereAR-style per-token VAE architecture
...
from cnn.spherear_model import SphereARVAE
...
def main(args):
    ...
    distributions = args.distributions if args.distributions else [
        "clifford", "powerspherical", "gaussian"
    ]
    ...
    train_set_full = datasets.CIFAR10("data", train=True, download=True, transform=transform)
    test_set_full = datasets.CIFAR10("data", train=False, download=True, transform=transform)
    ...
    if excluded_test_set is not None:
        excluded_test_loader = DataLoader(excluded_test_set, batch_size=args.batch_size, shuffle=False, num_workers=2)
        print(f"excluding class {args.exclude_class}. excluded test set size: {len(excluded_test_set)}")
    ...
    across_dim_results = {
        d: {"knn_100": [], "knn_600": [], "knn_1000": [], "f1_100": [], "f1_600": [], "f1_1000": [], "mean_cosine": [], "dims": []}
    ...
    for latent_dim in latent_dims:
        ...
        for dist_name in distributions:
            for trial in range(args.n_trials):
                ...
                exp_name = f"cifar10-{dist_name}-d{latent_dim}-{args.recon_loss}{trial_suffix}"
                ...
                # per-token latent dim: total ~ requested latent_dim
                # with 2 down blocks: 32->16->8, so 8x8=64 tokens
                num_tokens = 64
                token_dim = max(4, latent_dim // num_tokens)
                print(f"  {dist_name}: {num_tokens} tokens x {token_dim}d = {num_tokens * token_dim}d total")
                ...
                model = SphereARVAE(
                    ...
                    encoder_chs=[64, 128],  # 2 down blocks: 32->16->8
                    decoder_chs=[128, 64],
                ...
                if os.path.exists(f"{output_dir}/best_model.pt"):
                    ...
                    # role-filler unbinding with both * and † methods
                    print(f"running role-filler unbinding test ({dist_name})...")
                    deconv_dir = f"{output_dir}/deconv"
                    os.makedirs(deconv_dir, exist_ok=True)
                    rf_results = {}
                    rf_variants = [
                        (True, "*", output_dir, "role_filler_capacity"),
                        (False, "*", output_dir, "role_filler_no_random_keys"),
                    ...
                    fourier_metrics.update({
                        f"*/{k}": v for k, v in fourier_star.items()
                    ...
                    # plots
                    two_per_class_plot = os.path.join(output_dir, "bundle_similarity_matrix.png")
                    ...
                    for rf_name in ["role_filler_capacity", "role_filler_no_random_keys",
                                    "role_filler_capacity_deconv", "role_filler_no_random_keys_deconv"]:
                        save_d = deconv_dir if "deconv" in rf_name else output_dir
                    ...
                    print(f"eval time: {eval_time:.2f}s, total exp time: {exp_time:.2f}s")
                    ...
                    across_dim_results[dist_name]["knn_100"].append(knn_metrics.get("knn_acc_100", 0.0))
                    across_dim_results[dist_name]["knn_600"].append(knn_metrics.get("knn_acc_600", 0.0))
                    across_dim_results[dist_name]["knn_1000"].append(knn_metrics.get("knn_acc_1000", 0.0))
                    across_dim_results[dist_name]["f1_100"].append(knn_metrics.get("knn_f1_100", 0.0))
                    across_dim_results[dist_name]["f1_600"].append(knn_metrics.get("knn_f1_600", 0.0))
                    across_dim_results[dist_name]["f1_1000"].append(knn_metrics.get("knn_f1_1000", 0.0))
                    across_dim_results[dist_name]["mean_cosine"].append(float(mean_vector_acc))
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="cifar-10 VAE experiments with sphereAR S-VAE + baselines"
    )
    ...
    p.add_argument(
        "--no-l2_norm", dest="l2_norm", action="store_false",
    ...
    p.add_argument("--wandb_project", type=str, default="clifford-experiments-CNN-cifar10")
    ...
    p.add_argument(
        "--exclude_class", type=int, default=-1,
