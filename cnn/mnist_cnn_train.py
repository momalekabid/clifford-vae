...
def main(args):
    ...
    for dataset_name in datasets_to_test:
        ...
        # mnist is 28x28 -> pad to 32x32 so the existing 4-stride cnn lands on 2x2
    ...
    with open("mnist_cnn_train_timing.json", "w") as f:
        json.dump(timing_results, f, indent=2)
    print(f"\ntotal script execution time: {script_total_time:.2f}s")
    print(f"timing results saved to mnist_cnn_train_timing.json")


if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="clifford vae experiments on mnist (cnn arch)"
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=-1,
        help="exclude class (-1=none) | mnist: digit 0-9",
    )
    p.add_argument(
        "--latent_dims",
        type=int,
