...
def vis_tsne(model, data_loader, device, n=1000, perplexity=30, max_iter=1000, save=None):
    """
...
def run_experiment(z_dim, device, n_runs=N_RUNS):
    ...
    for run in range(n_runs):
        ...
        vis_random_samples(model, test_loader, device,
                        save=get_fig_path(z_dim, "random"))

        vis_tsne(model, test_loader, device,
                n=1500, save=get_fig_path(z_dim, "tsne"))

        vis_interp(model, test_loader, device,
                    save=get_fig_path(z_dim, "interpolated"))
